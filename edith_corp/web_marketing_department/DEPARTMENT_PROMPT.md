...
ローテーションから投稿日（火/木/土の日付）が決まったら、strategy.json の `rotation.publish_times` で公開時刻を取得し、予約投稿日時を計算します。

例: W1の木曜 → 2026-02-13 + publish_times.thu(07:00) → `scheduled_date: "2026-02-13T07:00:00"`

この `scheduled_date` をブログ事業部に渡してください。
...
- **scheduled_date**: Step 2.1aで計算した予約投稿日時
