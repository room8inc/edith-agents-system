...
- **scheduled_date**: 予約投稿日時（ISO 8601形式、例: `2026-02-13T07:00:00`）。省略時はドラフト投稿
...
- **毎回同じフレーズを使わない。** 以下のバリエーションからランダムに選び、1記事内で同じ表現を繰り返さないこと:
  - 切り込み系: 「身も蓋もない話をすると」「不都合な事実なんですけど」「これ、本当のこと言っていいですか」「はっきり言いますけど」「残酷な話をします」「これ、怒る人いると思うんですけど」
  - 共感誘導系: 「みんな気づいてるけど言わないだけで」「誰も指摘しないんですけど」「冷静に考えると怖い話で」「目を背けたくなる事実ですが」
  - まとめ系: 「ぶっちゃけ」「要するにそういうことなんですよ」「シンプルに言うと」「構造的な問題として」「つまりそういうことです」

**記事構成:**
- タイトル（SEOキーワード含む）
- リード文（**必ず「こんにちは、Room8の鶴田です！」で始める**。挨拶から自然に話題に入り、問題提起につなげる。唐突に切り替えず、話しかけるような流れで）
- 本文3-5セクション（H2/H3見出し）
- まとめ（実践的アクション）
- 文字数目安: 3,000-5,000文字
...
```json
{
  ...
  "author": "鶴田（Room8）",
  "created_at": "<ISO8601>",
  "category": "<下記の7カテゴリーから1つ選択>",
  "tags": ["<記事内容に関連するタグを3-5個>"],
  "seo": {
  ...
  "wordpress": {
    "status": "draft",
    "post_id": null,
    "scheduled_date": "<scheduled_dateが渡されていれば設定。なければこのキー自体を省略>"
  }
}
```

**カテゴリー選択ルール（必ずこの7つから1つ選ぶ）:**
- `AIラボ` — AI・ChatGPT・Gemini・Claude等のAI関連記事
- `Webラボ` — Web開発・Webツール関連
- `コワーキングスペース` — コワーキング・オフィス関連
- `マーケティング・SEO` — マーケティング・SEO・集客関連
- `働き方・生産性` — 働き方改革・生産性向上・業務効率化
- `財務・会計` — 財務・会計・経理関連
- `起業・個人事業主` — 起業・フリーランス・個人事業主関連

迷ったら `AIラボ` をデフォルトにしてください。

**タグ生成ルール:**
- 記事の主要キーワードから3-5個選ぶ
- ターゲットキーワードは必ず含める
- 例: `["ChatGPT", "AI活用", "業務効率化", "中小企業"]`
```
